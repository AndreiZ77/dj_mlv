...
class Post(models.Model):
    ...
    tags = models.ManyToManyField('Tag', blank=True,
                                  related_name='posts')  # явно указываем свойство для Tag: related_name='posts'
    date_pub = models.DateTimeField(
        auto_now_add=True)  # сохранение при записи в БД, auto_now изменение каждый раз при изменении объекта
    ...
    def __str__(self):
        return self.title  # возвратит заголовок экземпляра класса Post

    class Meta:
        ordering = ['-date_pub']  # обратная сортировка по дате
